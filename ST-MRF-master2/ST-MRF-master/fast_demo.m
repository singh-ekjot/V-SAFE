...
% init buffer
D = min(I,[],3);        %Eq.(2)
D = imResample(D, 1/s, 'nearest'); %fast downsample
...
for n = 1 : numel(img_files)
    if n<numel(img_files)
        ...
        refine_D = fast_stMRF(pre_gray, Dt, uD_t, lambda_t, D_r, s, eps); %Eq.(7)
    else
        Dt(:,:,1)=[];       %pop the old D_t
        Dt=cat(3,Dt,D);     %push the new D
        ...
        refine_D = fast_stMRF(pre_gray, Dt, uD_t, lambda_t, D_r, s, eps);
    end
    ...
    subplot(1,1,1);imshow(dehaze);title('ST-MRF');
    drawnow;
end
disp(['Fast ST-MRF Dehazing:' num2str(nFrames/time) 'fps']);
